<div>
    <div class="sm:px-6 w-full" x-data="{ open: false }">
                    ...
                    <tbody>
                        @foreach ($articles as $article)
                            <tr tabindex="0" class="focus:outline-none h-16 border border-gray-100  rounded">
                                ...
                                <td class="">
                                    <a href="{{ route('articles.show', $article->id) }}"
                                        class="flex items-center pl-5 hover:underline">
                                        <p class="text-base font-medium leading-none text-gray-700 mr-2">
                                            {{ $article->title }}</p>
                                        <img src="https://tuk-cdn.s3.amazonaws.com/can-uploader/tasks-svg1.svg"
                                            alt="clip">
                                    </a>
                                </td>
                                {{-- <td class="pl-24">
                            ...
                            <td class="pl-5">
                                ...
                                <td>
                                    <div class="relative px-5 pt-2 flex justify-end space-x-7">
                                        <button wire:click="startEdit({{ $article->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="icon icon-tabler icon-tabler-edit" width="28" height="28"
                                                viewBox="0 0 24 24" stroke-width="1" stroke="#5FA7DE" fill="none"
                                        ...
                                        <form action="{{ route('articles.destroy', $article->id) }}" method="POST">
                                            ...
                                            <button>
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    class="icon icon-tabler icon-tabler-trash" width="28"
                                                    height="28" viewBox="0 0 24 24" stroke-width="1" stroke="#9779F7"
                            ...
                            <tr>
                               ...
                               <livewire:article-edit :articleId="$article->id" :key="$article->id" >
                            </tr>
                            @endif
                        @endforeach
                        <div class="absolute top-0 left-0 flex flex-col items-center justify-center w-full h-full z-40"
                            style="background-color: rgba(0,0,0,.5);" x-show="open" x-transition>
                            <!-- A basic modal dialog with title, body and one button to close -->

                            <div class="relative h-5/6 p-4 text-left bg-white rounded shadow-xl w-full lg:w-3/6 overflow-y-scroll"
